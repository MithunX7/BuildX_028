...
export async function findNearbyDuplicateIssue(params: {
  ...
  try {
    // 1. Try MongoDB $near geospatial query on open / active issues
    ...
    if (candidate && candidate.location?.coordinates) {
      ...
      // Distance-based match confidence: closer = higher confidence
    ...
    // Fallback in-memory distance search if 2dsphere index is still building or in dev
